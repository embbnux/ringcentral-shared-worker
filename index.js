$(function() {
  ...
  async function initCallControl() {
    subscription = new RingCentral.SharedSubscription();
    rcCallControl = new RingCentralCallControl({ sdk: rcsdk });
    window.rcCallControl = rcCallControl;
    subscription.on('notification', function(msg) {
  ...
  async function show3LeggedLogin(server, clientId) {
    rcsdk = new RingCentral.SharedSDK({
    ...
    var loginUrl = await platform.loginUrlWithDiscovery({ usePKCE: true });
